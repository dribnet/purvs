...
function draw_clock(obj) {
    ...
    background(r,0,0);
        ...
        sunrise  = map(hourLoci,CANVAS_HEIGHT,0,200,0);
		
		
		if(hours>17){
			
		fill(210,210,0); //make a yellow for day and blue for night
    	ellipse(sunrise+15,120,50,50); //map it to go across screen and change colour 
    	fill(r,0,0);
    	ellipse(sunrise,110,50,50); //make a map and set the x value to it
   		}
   		else{
		fill(204, 102, 0);
    	ellipse(sunrise,120,60,60);
    	fill(210,210,0);
    	ellipse(sunrise,120,50,50);
        ...
        fill(r,0,0); //outta colour for circle
        ellipse(secLoci,startY+126,60,60); //outta circle
		}
		fill(255); //inner circle
		ellipse(secLoci,startY+126,50,50); //inner circle
        fill(0);
		...
		if(seconds_until_alarm !=0){
		fill(r,0,0); //outta colour for circle
        ellipse(minLoci,startY+63,60,60); //outta circle
		}
        fill(255);
		...
		if(seconds_until_alarm !=0){
		fill(r,0,0);
        ellipse(milLoci,startY+190,60,60);
		}
        fill(255);
