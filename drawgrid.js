...
function drawGrid(p5, x1, x2, y1, y2, z, zoom) {
  ...
  // A small white ellipse of size 0.5 inside the blue ellipse.
  cx = p5.map(512, x1, x2, 0, 256);
  cy = p5.map(512, y1, y2, 0, 256);
  cx2 = p5.map(512+0.5, x1, x2, 0, 256);
  p5.fill(255, 255, 255);
  p5.ellipse(cx, cy, (cx2-cx));

  // An even smaller black ellipse of size 0.01 inside the white ellipse.
  cx = p5.map(512, x1, x2, 0, 256);
  cy = p5.map(512, y1, y2, 0, 256);
  cx2 = p5.map(512+0.01, x1, x2, 0, 256);
  p5.fill(0);
  p5.ellipse(cx, cy, (cx2-cx));

  // An even smaller white ellipse of size 0.001 inside the black ellipse.
  cx = p5.map(512+0.002, x1, x2, 0, 256);
  cy = p5.map(512, y1, y2, 0, 256);
  cx2 = p5.map(512+0.001, x1, x2, 0, 256);
  p5.fill(255);
  p5.ellipse(cx, cy, (cx2-cx));
  ...
  cx = p5.map(512-0.002, x1, x2, 0, 256);
  cy = p5.map(512, y1, y2, 0, 256);
  cx2 = p5.map(512+0.0001, x1, x2, 0, 256);
  p5.fill(255);
  p5.ellipse(cx, cy, (cx2-cx));
