...
// Phoebe helped me with this array
  ...
  let hourArcLegth = map(obj.hours, 0, 23, 0, 12.56) // weird precision
  let minArcLegth = map(obj.minutes, 0, 59, 0, 6.28) // weird precision
  push();
  strokeJoin(ROUND);
  strokeWeight(30);
  noFill();
  translate(490, 400);
  line(0, 0, 1000, 1)
  stroke(customColours[minutesColourMap]); // colour changing map
  translate(-10, -150)
  rotate(-3.14 / 2)
  arc(0, 0, 370, 370, 0, minArcLegth);
  pop();
  // end of phoebes code
  ...
  textSize(80);
  fill(255, 179, 110); // lolly orange
  text(obj.seconds, 150 + moveSecondsText, 100); // text placement
  pop();
  ...
  if (alarm < 0) {
    ...
    // bright red, bt slight transparent
    fill(255, 0, 0, 100);
    ellipse(480, 250, 0 + millis / 2); // big old alarm circle radar thing
